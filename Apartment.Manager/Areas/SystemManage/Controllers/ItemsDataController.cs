...
using System.Xml;
using System;
using Apartment.Model.Enum.Order;
...
namespace Apartment.Manager.Areas.SystemManage.Controllers
{
    public class ItemsDataController : ControllerBase
    {
        ...
        /// <returns></returns>
        public ActionResult GetEnum(string enCode)
        {
            List<object> list = new List<object>();
            if (enCode == "OrderStatus")
            {
                foreach (var bt in Enum.GetValues(typeof(OrderStatus)))
                {
                    list.Add(new { id = (int)bt, text = bt.ToString() });
                }
            }
            if (enCode == "LookStatus")
            {
                foreach (var bt in Enum.GetValues(typeof(LookStatus)))
                {
                    list.Add(new { id = (int)bt, text = bt.ToString() });
                }
            }
            if (enCode == "IsAgree")
            {
                foreach (var bt in Enum.GetValues(typeof(IsAgree)))
                {
                    list.Add(new { id = (int)bt, text = bt.ToString() });
                }
            }
